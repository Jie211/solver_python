...
def cg_method_cupy(a_mat, b_vec, max_iter=10000, epsilon=1e-10, logger_level=logging.INFO):
    # init
    x_vec = cp.zeros(b_vec.shape)
    b_norm = cupy_norm(b_vec, ord=2)
    is_converged = False
    k = 0
    residual = []
    step = max_iter / 10
    ...
    while k < max_iter:
        ...
        res = cupy_norm(r_vec, ord=2) / b_norm
        residual.append(res)
        if k % step == 0:
